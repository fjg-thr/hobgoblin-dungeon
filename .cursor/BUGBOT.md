...
- `src/app/layout.tsx` references `/opengraph-image.png` with dimensions
  `1360x752`. As of this deployment there is no tracked
  `public/opengraph-image.png`; do not block unrelated PRs for the existing
  baseline, but flag metadata/share-image changes that add broken public asset
  references, remove referenced assets, or drift from expected dimensions or alt
  text.
