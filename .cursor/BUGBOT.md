...
- The React browser-only Phaser mount point lives in `src/game/GameCanvas.tsx`.
- App route metadata, including OpenGraph and Twitter share image metadata, lives in `src/app/layout.tsx`.
...
- Review route metadata changes for deploy-safe URLs, stable image dimensions, useful alt text, and compatibility with both local development and Vercel environments.
...
- If metadata or share images change, verify the referenced assets exist under `public` and that image dimensions match the metadata.
