...
## Hosted Bugbot enablement checklist

- Install or update the Cursor GitHub App so it can access
  `fjg-thr/hobgoblin-dungeon`.
- Enable Bugbot for this repository in the Cursor dashboard, or use the Cursor
  Admin API with `repoUrl` set to
  `https://github.com/fjg-thr/hobgoblin-dungeon` and `enabled` set to `true`.
- Configure the organization/repository preference for automatic PR reviews and
  whether unresolved Bugbot findings should fail the `Cursor Bugbot` check.
- If branch protection should require automated review, require the GitHub check
  named `Cursor Bugbot`.
