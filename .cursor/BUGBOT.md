...
- Browser globals (`window`, `document`, canvas, pointer APIs, local storage,
  Web Audio) should only run from client-only code paths.
...
- Keep controls and docs aligned. Review diffs against `createInput`, pointer
  handlers, the how-to-play overlay, and `README.md` so movement, aiming,
  firing, mute, and debug controls do not drift from what players see.
