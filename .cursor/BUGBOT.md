...
This file gives Cursor Bugbot repo-specific context after it is merged to the
default branch. Enabling the managed Bugbot service is controlled outside this
repo through Cursor org settings and GitHub App repository access; this file
does not prove that the hosted reviewer is enabled.
...
5. Map changes should update layout generation, collision, rendering, and
   ...
   codes are primarily collision/shape data and are not all visibly rendered.
6. React/Next changes should preserve SSR safety for Phaser and keep DOM UI
   styling aligned with existing `src/app/globals.css`; this repo is not using
   Tailwind or ShadCN.
...
- Existing dependency audit findings may appear during install; separate
  dependency hardening from unrelated gameplay or Bugbot-guidance changes.
...
manifest paths, frame dimensions, animation math, and runtime loadability rather
than pixel-perfect art critique.
