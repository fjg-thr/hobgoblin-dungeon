...
Review pull requests for correctness regressions in this Next.js and Phaser dungeon prototype. Prioritize issues that would break gameplay, rendering, asset loading, build output, metadata generation, input handling, or runtime stability. Keep comments actionable and avoid broad style feedback unless it hides a likely bug.
...
- Phaser scene changes that leak event listeners, keyboard handlers, pointer handlers, timers, delayed calls, tweens, sounds, sprites, containers, graphics objects, or shutdown handlers across scene restarts.
- Start screen, how-to-play modal, mute button, keyboard, or pointer input changes that can leave stale hit bounds, block starting a run, or fire gameplay actions while an overlay is open.
- Asset-manifest changes that do not update all coupled usage:
  ...
  - union-derived keys such as `TileAssetKey`, `AudioAssetKey`, actor names, projectile kinds, pickup kinds, and power-up names.
...
- Game logic changes that can create impossible states: negative health or ammo, seeker ammo above cap, unbounded spawn/timer growth, projectiles that never despawn, enemies attacking after death, pickups spawning inside blocked tiles, or power-up timers stacking incorrectly.
- Projectile and ammo changes that break standard staff bolts, seeker targeting/turning, blast interactions, projectile max distance, or despawn-on-impact behavior.
- Audio or mute changes that can start duplicate loops, ignore the mute state, play sounds while muted, or leave sound instances alive after restart.
