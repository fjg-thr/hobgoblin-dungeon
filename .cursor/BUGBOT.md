...
- React 19, Next 16, TypeScript strict mode, and Phaser `4.0.0-rc.4`.
...
- `npx tsc --noEmit --incremental false` is the explicit source type check and
  avoids writing `tsconfig.tsbuildinfo`.
...
- Binary runtime assets may be generated outside this checkout. This workspace
  can contain only JSON sidecars under `public/assets/**`; if PNG or WAV files
...
- Initial goblins are seeded from the dungeon start data; additional goblins
  ramp with target enemy count, and brutes unlock after configured progression
  gates.
...
- `src/game/GameCanvas.tsx` must keep Phaser behind dynamic client-side imports;
  do not import Phaser from server components.
- React Strict Mode is enabled in `next.config.ts`; review scene creation,
  listener registration, and `game.destroy(true)` cleanup for double-mount
  safety.
- `src/game/scenes/DungeonScene.ts` owns combat, input, HUD, audio, pickups,
  start/help/game-over UI, resize handling, and scene reset behavior.
...
- `POWERUP_CONFIG` controls unlock gates, spawn weights, and presentation
  metadata for quickshot, haste, ward, and blast. Effect timing also depends on
  nearby constants and collection logic.
...
- Metadata JSON under `public/assets/**` must match the generated image layout,
  but metadata alone does not load assets at runtime.
- Several wall tile assets are declared for collision or future rendering even
  when current map rendering skips blocked wall cells. Review tile changes
  against both rendering and blocking semantics.
