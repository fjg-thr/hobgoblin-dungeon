...
Admin API/team configuration, and a live PR smoke review after this file is on
the default branch.
...
3. For canvas/UI changes, verify pointer zones, keyboard affordances, resize
   behavior, HUD placement, and readable contrast at small viewport sizes.
...
6. For app metadata/share changes, note that `src/app/layout.tsx` references
   `/opengraph-image.png`. Treat the current missing file as a baseline issue,
   but block PRs that touch share metadata and leave it broken or worse.
