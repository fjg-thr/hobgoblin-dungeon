...
- This is a Next.js/React/TypeScript prototype with a Phaser game mounted
  through `src/game/GameCanvas.tsx` and primary gameplay in
...
- There is no Tailwind setup. DOM and metadata changes should follow semantic
  ...
  reviewed as canvas interactions: pointer hit areas, keyboard/mouse
  affordances, responsive placement, readable overlays, and accessibility limits
  of canvas UI.
...
- For `DungeonScene.ts`, review gameplay state invariants around health, ammo,
  ...
  depth ordering, camera resize behavior, and game-over/start-screen
  transitions.
...
or misleading docs. Include exact file and line references. Keep summaries
brief, and separate pre-existing baseline caveats from issues introduced by the
PR.
