...
- Treat this file as review guidance only. Managed Bugbot enablement still
  ...
  optional Admin API credentials, and a live PR review smoke check after the
...
- Manual GitHub PR review triggers should be top-level comments such as
  ...
  `bugbot run verbose=true` only when diagnostic request IDs or detailed
  service logs are needed.
- A PR that adds or edits this file may not be reviewed with the new rules
  until after the change lands on the default branch.
...
- `src/app/layout.tsx` defines page metadata and currently references
  `/opengraph-image.png`; `public/opengraph-image.png` should remain present
  when metadata/share image changes are made.
...
- Input and UX: runtime firing is bound to `SPACE` and pointer/click firing;
  README still mentions `J`, which is an existing docs mismatch. Only block a
  PR for that mismatch when the PR changes controls, input docs, or claims to
  fix control behavior.
...
- Audio: ensure additions are loaded through `assetManifest.audio`, handled by
  the scene mute/toggle behavior, and do not autoplay outside user-initiated game
  flow in ways browsers will block.
- Metadata/Next changes: preserve `metadataBase` behavior for Vercel/local
  URLs, keep share image dimensions/path accurate, and avoid client-only APIs in
  server components.
