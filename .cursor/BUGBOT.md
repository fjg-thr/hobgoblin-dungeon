...
This repository is a Next.js App Router shell that boots a client-side Phaser
dungeon prototype. Use this file as repository-specific review context for
Cursor Bugbot.
...
- `src/game/scenes/DungeonScene.ts` contains nearly all runtime behavior:
  preload/create/update, rendering, input, combat, enemy AI, pickups, power-ups,
  HUD, start/game-over flows, debug overlay, audio, and restart cleanup.
...
- Check `GameCanvas` changes for double-mount behavior under
  `reactStrictMode`, cancellation during dynamic import, and
...
If verification rewrites generated Next files such as `next-env.d.ts` or
creates `tsconfig.tsbuildinfo`, those artifacts should remain clean unless the
