...
This repository includes project-specific Bugbot review rules in `.cursor/BUGBOT.md`.

## Enable Bugbot for pull request reviews

Bugbot is enabled outside the repository through Cursor's GitHub integration:

1. Install or verify that the Cursor/Bugbot GitHub app has access to `fjg-thr/hobgoblin-dungeon`.
...
The API key is intentionally not stored in this repository. Run the command from a secure admin environment.
