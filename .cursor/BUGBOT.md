...
- The app shell is Next.js with React and TypeScript.
...
6. For UI or DOM controls, check keyboard access, visible labels, and appropriate ARIA attributes.
7. For audio changes, preserve mute behavior and avoid starting overlapping loops after scene restarts.
8. For TypeScript changes, prefer explicit narrow types and avoid weakening strictness with `any`, unsafe casts, or broad nullable state.
...
- `npm run build`
- `npm run lint` if the script is available and supported by the installed Next.js version
