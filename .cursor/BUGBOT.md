...
- Preserve pixel-art scaling, nearest-neighbor asset assumptions, and the intentionally compact prototype structure.
...
1. Flag changes that can break `npm run build` or TypeScript validation with `npx tsc --noEmit`.
...
- For app or TypeScript changes, expect `npx tsc --noEmit` and `npm run build` to pass.
