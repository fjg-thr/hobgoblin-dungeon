...
- Metadata changes in `src/app/layout.tsx` should keep referenced static files
  present under `public/`, with matching dimensions and useful alt text. Current
  main references `/opengraph-image.png`; if a PR changes share-image metadata
  or related image assets, verify that it preserves or intentionally fixes that
