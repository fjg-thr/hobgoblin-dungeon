...
- When validating deployment outside code, confirm the Cursor GitHub integration, Bugbot settings page, and a pull request review smoke check if those controls are available.

## Project context

- This is a private npm-package Next.js App Router prototype for a dark GBA-inspired isometric dungeon game.
- React owns page/layout composition while Phaser owns the game loop, scene, sprites, input, and audio state.
- Preserve pixel-art scaling and nearest-neighbor assumptions for generated assets.
- The app uses plain global CSS in `src/app/globals.css`; avoid styling-system churn unless the PR is explicitly about that migration.
...
3. Check gameplay state transitions around player death, restart, power-up effects, enemy spawning, projectile cleanup, and pickup collection.
...
- `src/app/layout.tsx` references `/opengraph-image.png`; preserve share metadata unless a PR intentionally changes metadata behavior.
