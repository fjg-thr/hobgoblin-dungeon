...
that the managed Cursor Bugbot service is enabled for the repository.
...
- Metadata and share-image changes in `src/app/layout.tsx` must keep referenced
  public files, dimensions, and alt text aligned. The current default branch
  references `/opengraph-image.png` without a matching public file, so do not
  block unrelated PRs for that existing condition; do flag metadata/share-image
  PRs that worsen it or claim to fix it without adding the asset.
