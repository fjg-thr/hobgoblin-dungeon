...
This repository is a Next.js app that hosts a Phaser-powered, browser-only dungeon game. Bugbot should focus on defects that could break gameplay, rendering, build output, asset loading, or reviewability.

## Project Context

- `src/app/page.tsx` mounts the game canvas through the app router.
- `src/game/GameCanvas.tsx` is the client-only boundary that dynamically imports Phaser and the dungeon scene.
...
- `src/game/maps/startingDungeon.ts` generates the dungeon layout and collision-relevant tile codes.
- `src/game/assets/manifest.ts` is the source of truth for public asset paths, sprite dimensions, animation metadata, and audio keys used by the scene.
- Generated image, audio, and JSON assets live under `public/assets/**`; asset processing and generation tools live under `tools/**` and `scripts/**`.
...
- Add a blocking Bug when `npm run build` would fail or when `package-lock.json` and `pnpm-lock.yaml` are updated inconsistently.
- Add a non-blocking Bug when a dependency is added but the change does not explain why the existing Next.js, React, Phaser, TypeScript, or standard browser APIs are insufficient.

## Verification Expectations

Ask authors for evidence appropriate to the diff:

- `git diff --check "$(git merge-base HEAD origin/main)"..HEAD`
- `npm ci`
- `npm run build`

For gameplay changes, also ask for a manual smoke pass covering start screen, how-to-play modal, WASD/arrow movement, pointer aiming, click firing, `SPACE` firing, ammo pickups, at least one power-up, player damage, death/game-over, restart, and mute toggle.
