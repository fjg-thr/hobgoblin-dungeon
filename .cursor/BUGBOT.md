...
- `src/game/scenes/DungeonScene.ts` contains most runtime state: input, combat,
  ...
  scene cleanup.
- `src/game/maps/startingDungeon.ts` generates rooms and corridors, assigns
  player/enemy starts, and defines tile blocking semantics.
...
- Metadata image paths must match committed files under `public/` or generated
  Next metadata route files. In particular, `src/app/layout.tsx` references
  `/opengraph-image.png`; changes near metadata should ensure that asset exists
