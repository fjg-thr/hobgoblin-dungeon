...
- The metadata in `src/app/layout.tsx` currently references
  `/opengraph-image.png`, but that public asset is not present in this checkout.
  Treat that as existing repository context unless a PR touches metadata, share
  images, public assets, or deployment configuration.
...
Repository files can provide review instructions and smoke-test commands, but
they cannot prove that the managed Cursor Bugbot service is enabled. For
deployment verification, confirm:

- Cursor dashboard or organization settings have Bugbot enabled for this
  repository.
- The Cursor/GitHub App has access to `fjg-thr/hobgoblin-dungeon`.
- A test pull request receives a Bugbot review or expected Bugbot status when
