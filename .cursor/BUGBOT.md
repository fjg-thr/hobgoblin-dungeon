...
- Preserve the client-only Phaser boot path. Runtime Phaser and scene modules should be loaded through the dynamic client-side boot flow in `src/game/GameCanvas.tsx`, and browser globals should stay behind client/effect boundaries.
...
- If lint tooling is updated or restored, run the project's lint command for lintable source changes.
