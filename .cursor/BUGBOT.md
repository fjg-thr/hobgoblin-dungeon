...
Bugbot always includes the root `.cursor/BUGBOT.md` file for reviews and can
also include nested `subdir/.cursor/BUGBOT.md` files for changes under that
subtree. Team, repository, project, and user rules are combined for each
review.
...
- `src/app/layout.tsx` owns metadata, OpenGraph, Twitter card, and
  environment-derived `metadataBase` behavior.
- `src/game/scenes/DungeonScene.ts` owns most runtime gameplay, including
  loading, animations, input, combat, pickups, HUD, audio, and scene cleanup.
- `src/game/assets/manifest.ts` is the runtime asset source of truth. Keep
  `public/assets/**` JSON manifests and `public/assets/audio/audio-manifest.json`
  consistent with code when assets change.
- `src/game/maps/startingDungeon.ts` defines map/tile data consumed by
  collision, depth sorting, and spawn logic.
...
4. **Assets and generated files:** when sprite sheets, manifests, audio, share
   images, or asset tooling change, require matching dimensions/keys/metadata
   and relevant generator or processor commands.
5. **DOM metadata/UI:** for App Router metadata or future HTML UI, prefer
   semantic elements and existing `src/app/globals.css` patterns. This repo
   does not currently use Tailwind or ShadCN.
...
- `src/app/layout.tsx` references `/opengraph-image.png`, backed by
  `public/opengraph-image.png`; require matching dimensions, alt text, and
  metadata if share-image or metadata files change.
