...
1. **Next.js and React boundaries**
   ...
   - Check metadata changes in `src/app/layout.tsx`; if OpenGraph or Twitter image paths change, verify the referenced files exist under `public`.
...
5. **Dependencies and lockfiles**
   - Dependency changes should be reflected in the appropriate lockfile generated by the selected package manager. This repo currently contains both `package-lock.json` and `pnpm-lock.yaml`, so review lockfile churn carefully.
