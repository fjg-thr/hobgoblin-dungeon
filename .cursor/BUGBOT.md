...
This deployment pins package versions so npm and pnpm resolve the same dependency graph in CI. Next.js is pinned to the audited `16.2.6` release and `postcss` is overridden to `8.5.15`; treat those pins as dependency hardening.

## Project map

- `src/app/page.tsx` mounts the client-only game canvas.
- `src/app/layout.tsx` defines site metadata and references the tracked `public/opengraph-image.png` share image.
