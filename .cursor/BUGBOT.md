...
- Existing context: README mentions `Space` or `J` for firing, while the current
  runtime binds keyboard firing to `SPACE` and supports pointer/click firing.
  Do not block unrelated PRs solely for that existing mismatch, but flag changes
  that make control copy less accurate.
