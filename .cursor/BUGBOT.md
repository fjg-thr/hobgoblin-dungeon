...
- Metadata changes in `layout.tsx` should keep `metadataBase`, OpenGraph, and Twitter image definitions consistent with files committed under `public`.
...
- Controls documentation, start/how-to-play text, and actual Phaser input bindings should stay aligned when any of those surfaces change.
...
- `/opengraph-image.png` is referenced by both OpenGraph and Twitter metadata. If its dimensions, path, or alt text change, `src/app/layout.tsx` and the committed public asset should be updated together.
- Treat newly introduced missing or stale social preview references as deploy-impacting because metadata references are resolved at build/runtime.
