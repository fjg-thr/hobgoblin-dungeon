...
prototype. Prioritize runtime regressions and review comments that preserve the
current Next.js + Phaser game behavior.

## Project context

- This is a Next.js app that mounts a Phaser 4 release-candidate game
  client-side.
...
- `src/app/layout.tsx` declares Open Graph and Twitter metadata. When metadata
  is touched, verify every referenced share image exists in `public/` or the
  change intentionally adds a generated Next.js metadata image route.
- If the app references `/opengraph-image.png`, require a matching
  `public/opengraph-image.png` file or an equivalent app metadata image route.
- Metadata code must stay server-safe and must not import Phaser or game
  modules.
...
- This repo has both `package-lock.json` and `pnpm-lock.yaml`. Flag dependency
  changes that update only one lockfile unless the change explicitly
  standardizes on a package manager.
