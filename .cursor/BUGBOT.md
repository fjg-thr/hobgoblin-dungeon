...
- `src/game/scenes/DungeonScene.ts` owns preload, animations, combat, HUD,
  input, audio, powerups, debug, layout, and restart.
...
1. Asset changes must keep `src/game/assets/manifest.ts`, committed files in
   `public/assets/**`, and any sidecar JSON in sync. New manifest paths without
...
- Manual browser smoke test for Phaser boot, restart, movement, firing,
  pickups, mute, and responsive canvas behavior when gameplay or UI is touched
