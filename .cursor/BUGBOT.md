...
- This file supplies review instructions only. Managed Bugbot enablement still depends on Cursor dashboard/org settings, GitHub App repository access, service configuration, Admin API credentials if used, and a live PR review smoke check after merge.
- Manual PR review triggers are top-level PR comments: `cursor review` or `bugbot run`. For diagnostics/request IDs/log detail, use `cursor review verbose=true` or `bugbot run verbose=true`.
- If hosted service state is not visible from the PR, say so. Do not claim Bugbot is enabled solely because this file exists.
...
- `DungeonScene.ts` owns gameplay, HUD overlays, input, audio, pickups, enemies, projectiles, and asset preload calls.
- `src/game/maps/startingDungeon.ts` owns dungeon generation, tile codes, collision decisions, props, and spawn placement.
- `src/game/assets/manifest.ts` is the runtime source of truth for loaded visual and audio assets. `public/assets/audio/audio-manifest.json` is auxiliary consistency data.
...
- For audio changes, verify new sounds are preloaded from `assetManifest.audio`, respect the scene-level mute toggle, avoid autoplay outside browser-allowed paths, and keep `audio-manifest.json` consistent when intentionally maintained.
...
- `src/app/layout.tsx` references `/opengraph-image.png`, but no tracked `public/opengraph-image.png` exists. Flag PRs that add or worsen broken metadata references, remove referenced public assets, or claim to fix metadata without adding the asset; do not block unrelated PRs for this baseline issue.
...
- Generated sprite/audio changes should include source, metadata, and runtime manifest updates together when applicable.
...
- If local verification cannot prove a managed Bugbot deployment, report repository-file verification separately from the external hosted-service smoke check.
