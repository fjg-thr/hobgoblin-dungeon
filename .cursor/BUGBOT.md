...
Review pull requests for correctness regressions in this Next.js and Phaser dungeon prototype. Prioritize issues that would break gameplay, rendering, asset loading, build output, or runtime stability. Keep comments actionable and avoid broad style feedback unless it hides a likely bug.
...
- Changes to `GameCanvas.tsx` that remove the dynamic `import("phaser")`, duplicate-boot guard, cancellation handling, or `destroy(true)` cleanup without an equivalent lifecycle-safe replacement.
...
- Asset path changes that reference files outside `public/assets` or mismatch checked-in PNG, WAV, or JSON files.
- Game logic changes that can create impossible states: negative health or ammo, unbounded spawn or timer growth, projectiles that never despawn, enemies attacking after death, or power-up timers stacking incorrectly.
