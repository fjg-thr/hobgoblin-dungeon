...
This repository-specific guide gives Cursor Bugbot context for reviewing pull
requests in `fjg-thr/hobgoblin-dungeon`.

Managed Bugbot enablement happens outside this repository through the Cursor
dashboard and GitHub App repository access. This file supplies project review
guidance once Bugbot runs; it does not enable the managed service by itself.

Manual review triggers on pull requests:

- `cursor review`
- `bugbot run`
- `cursor review verbose=true` or `bugbot run verbose=true` for
  troubleshooting
...
5. User-facing behavior
   ...
   - If README behavior is intentionally changed, update the README in the same
     PR.
...
- `src/app/layout.tsx` references `/opengraph-image.png` for share metadata,
  but no tracked file with that name exists in the current tree. Flag this only
  when a PR changes metadata, share-card assets, or public static file handling.
