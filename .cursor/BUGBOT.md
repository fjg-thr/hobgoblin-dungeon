...
Use this context when reviewing pull requests for this repository. Prioritize correctness, runtime regressions, asset consistency, and high-signal review comments over style-only feedback.

## Project context

- This is a Next.js App Router prototype for a browser-only Phaser 4 dungeon game.
- `src/app/page.tsx` renders the client game entrypoint; `src/game/GameCanvas.tsx` dynamically imports and boots Phaser on the client.
...
- Static sprites, tilemaps, UI panels, share images, and audio live under `public/`, primarily `public/assets/`.
...
2. **Gameplay correctness**
   ...
   - Current player controls are `WASD`/arrows for movement, mouse or pointer aiming, `Space` shooting, and click-to-fire. The README also mentions `J`; treat that as existing documentation drift unless the PR is about controls or docs.
...
3. **Map, camera, and collision changes**
   - Confirm tile coordinate math, isometric projection, depth sorting, wall collision, bridge and stair behavior, camera bounds, and debug overlay updates stay consistent.
...
4. **Assets and manifests**
   ...
   - Do not request manual edits to generated sprite sheets when the generator or processor source should change instead.
...
5. **UI, metadata, and accessibility**
   ...
   - For Phaser canvas UI, focus on pointer zones, keyboard and mouse affordances, readable placement at different viewport sizes, and whether canvas-only interactions need mirrored DOM affordances.
   - `src/app/layout.tsx` references `/opengraph-image.png`; the tracked repo may not include that asset. Flag metadata or share-image changes that add broken references or worsen dimensions or alt text, but do not block unrelated PRs solely for the existing baseline.
...
6. **Dependencies and verification**
   ...
   - `next lint` is not reliable for this project baseline. Prefer `npm run build` and `npx tsc --noEmit --incremental false` for TypeScript and build verification when runtime code changes.
