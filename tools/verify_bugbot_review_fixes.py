...
def verify_mute_button_pointer_does_not_queue_shot() -> None:
    ...
    assert_condition(
        "this.isPointerOverMuteButton(pointer)" in handle_pointer_body,
        "Global pointer shooting should check whether the pointer is over the mute button",
    )
    assert_condition(
        handle_pointer_body.find("this.isPointerOverMuteButton(pointer)")
        < handle_pointer_body.find("this.shotQueued = true"),
        "Mute button pointer guard must run before a shot is queued",
    )
