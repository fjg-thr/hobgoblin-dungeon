...
1. **Next.js and React boundaries**
   ...
   - Check metadata changes in `src/app/layout.tsx`; if OpenGraph or Twitter image paths change, verify the referenced files exist under `public` or are provided by a valid App Router metadata route. The current metadata references `/opengraph-image.png`, which is tracked at `public/opengraph-image.png`.
